const CACHE='absen-sppg-v23-1';
const SHELL=['./','./index.html','./supabase-config.js','./security-ops-client.js','./src/app/bootstrap.js','./src/styles/app-system.css','./src/styles/mobile-compact-hotfix.css'];
...
self.addEventListener('fetch',event=>{
 ...
 if(networkFirst){event.respondWith(fetch(request,{cache:'no-store'}).then(response=>{if(response.ok){const copy=response.clone();caches.open(CACHE).then(cache=>cache.put(request,copy));}return response;}).catch(()=>caches.match(request).then(cached=>cached||caches.match('./index.html'))));return;}
