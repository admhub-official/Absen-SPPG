...
async function authenticate(token: unknown) {
  ...
  const { data: session, error: sessionError } = await supabase.from("Sessions").select("ID_User,Type,Expires_At").eq("Token", cleanToken).maybeSingle();
  ...
  const { data: user, error: userError } = await supabase.from("Users").select("ID_User,Email,Role,SPPG,Status_Aktif").eq("ID_User", session.ID_User).maybeSingle();
...
async function scopedUserIds(auth: any): Promise<string[]> {
  ...
  const { data: accessRows, error: accessError } = await supabase.from("Akses_Email").select("SPPG,Aktif").ilike("Email", String(auth.Email || ""));
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!["HISTORY", "DITERBITKAN", "MENUNGGU_TTD_PENERIMA", "DRAFT"].includes(status)) throw new Error("Status slip tidak valid");
    if (!userIds.length) return json({ success: true, items: [], total: 0, page, pageSize, totalPages: 0 });

    const from = (page - 1) * pageSize;
    const to = from + pageSize - 1;
    let query = supabase.from("Slip_Gaji")
    ...
    const orderColumn = status === "DRAFT" ? "Created_At" : "Diterbitkan_At";
    const { data: slips, error, count } = await query.order(orderColumn, { ascending: false, nullsFirst: false }).order("ID_Slip", { ascending: true }).range(from, to);
    if (error) throw error;
