...
const CORE_URL = `${SUPABASE_URL}/functions/v1/AbsenCore`;
...
async function superAdminOverview(user: any) {
  ...
  const presentAll = new Set(attendance
    .map((row: any) => String(row.ID_User))
    .filter((id: string) => activeUsers.some((userRow: any) => String(userRow.ID_User) === id)));
...
async function geofenceRequired() {
...
async function handleGeofenceDisabled(body: any, request: Request) {
  const functionName = String(body.function || body.functionName || '');
  if (!['checkAttendanceLocation', 'recordAbsensiSelf'].includes(functionName)) return null;
  if (await geofenceRequired()) return null;

  const data = body.data || body;
  const user = await authenticate(data.token);
  if (data.idUser && String(data.idUser) !== String(user.ID_User)) {
    throw new Error('Akses ditolak. Identitas absensi tidak sesuai dengan sesi login.');
  }

  if (functionName === 'checkAttendanceLocation') {
    return json({
      success: true,
      result: {
        valid: true,
        ...
        jarak: null,
        radius: null,
        referenceName: 'Kebijakan sistem',
        geofenceRequired: false,
      },
    });
  }

  const core = await forward(CORE_URL, body, request);
  return new Response(core.text, { status: core.status, headers });
}
...
Deno.serve(async (request) => {
  ...
  try {
    const geofenceResponse = await handleGeofenceDisabled(body, request);
    if (geofenceResponse) return geofenceResponse;
