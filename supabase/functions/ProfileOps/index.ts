...
type SessionUser = {
  ID_User: string;
  Role: string | null;
  Status_Aktif: boolean | string | number | null;
};
...
async function resolveFoundation(sppg: string, requestedFoundation: string): Promise<string> {
  ...
  const master = await db.from("Master_SPPG")
    .select("Nama_SPPG,Yayasan,Aktif")
    .ilike("Nama_SPPG", sppg)
    .limit(1)
    .maybeSingle();
  if (!master.error && master.data && active(master.data.Aktif)) {
    return clean(master.data.Yayasan, 180) || requestedFoundation;
  }
  return requestedFoundation;
}

async function updateProfile(user: SessionUser, updatesValue: unknown) {
  const updates = updatesValue && typeof updatesValue === "object"
  ...
  if (Object.prototype.hasOwnProperty.call(updates, "Role") || Object.prototype.hasOwnProperty.call(updates, "Status_Aktif")) {
  ...
  const stringFields: Array<[string, number]> = [
    ...
    ["Nama_Bank", 120],
    ["Nomor_Rekening", 80],
    ["Atas_Nama_Rekening", 180],
    ["SPPG", 160],
    ["Yayasan", 180],
    ["Jabatan_Divisi", 180],
  ];
  for (const [field, max] of stringFields) {
    if (updates[field] !== undefined) payload[field] = clean(updates[field], max);
  }
  ...
  if (!clean(payload.Nama_Lengkap ?? updates.Nama_Lengkap, 180) && updates.Nama_Lengkap !== undefined) {
  ...
  if (payload.SPPG !== undefined || payload.Yayasan !== undefined) {
    payload.Yayasan = await resolveFoundation(sppg, requestedFoundation);
  }
  ...
  try {
    await db.from("Audit_Log").insert({
      ...
      Detail: {
        ...
        protectedFields: ["Gaji_Harian", "Role", "Status_Aktif"],
      },
    });
  } catch (error) {
    console.warn("Profile audit deferred", error);
  }

  return { success: true, message: "Profil dan data kepegawaian berhasil diperbarui." };
}
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (action !== "updateProfil") return json({ success: false, error: "Aksi profil tidak didukung." }, 422);
    const user = await authenticate(body.token);
    const result = await updateProfile(user, body.updates);
    ...
    const status = /tidak valid|tidak boleh|hanya dapat|Format|Tanggal/i.test(raw) ? 422 : 500;
