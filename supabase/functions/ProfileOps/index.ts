...
async function updateProfile(user: SessionUser, updatesValue: unknown) {
  ...
  try {
    const audit = await db.from("Audit_Log").insert({ ID_Log:`AUD_${Date.now()}_${crypto.randomUUID().slice(0,8)}`, Waktu:new Date().toISOString(), ID_User_Pelaku:user.ID_User, Jenis_Aktivitas:"UPDATE_PROFIL", Detail:{ idUser:user.ID_User, fields:Object.keys(payload).filter((field)=>field!=="Updated_At"), protectedFields:["Gaji_Harian","Role","Status_Aktif"] } });
    if (audit.error) throw audit.error;
  } catch (error) { console.warn("Profile audit deferred", error); }
  return { message:"Profil dan data kepegawaian berhasil diperbarui.", profile:await getProfile(user) };
}
...
function mapError(raw: string) {
  ...
  if (/tidak valid|tidak boleh|hanya dapat|Format|Tanggal|NIK|wajib berupa object|Tidak ada field/i.test(raw)) return { status: 422, code: "VALIDATION_ERROR", message: raw };
