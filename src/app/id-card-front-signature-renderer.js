(() => {
  ...
  async function paintPair(pair, force = false) {
    ...
    // Area bawah master depan memang disediakan sebagai ruang TTD.
    ctx.save();
  ...
  function schedule() {
    ...
    requestAnimationFrame(() => requestAnimationFrame(async () => {
      scheduled = false;
      for (const pair of document.querySelectorAll('.digital-id-preview-pair')) {
        await paintPair(pair);
      }
    }));
  }
  ...
  // Renderer master membuat PDF dari canvas yang sama. Pastikan TTD depan sudah dicat sebelum handler export berjalan.
  ...
  document.addEventListener('click', (event) => {
    const button = event.target.closest?.('[data-front-signature-replay="1"]');
    if (button) delete button.dataset.frontSignatureReplay;
  }, true);
