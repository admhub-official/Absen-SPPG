...
Deno.test("phase 8A migration establishes backward-compatible session hashes", async () => {
  ...
  for (const marker of [
    ...
    if (!migration.includes(marker)) throw new Error(`session hash foundation missing ${marker}`);
  }
});
...
Deno.test("final migration persists only SHA-256 digests while retaining the legacy PK alias", async () => {
  ...
  for (const marker of [
    ...
    if (!migration.includes(marker)) throw new Error(`final session migration missing ${marker}`);
  }
  if (/drop\s+constraint\s+"?Sessions_pkey"?/i.test(migration)) {
    throw new Error("legacy Token PK must remain available as a digest-only compatibility alias");
  }
});
...
Deno.test("shared modern authentication is hash-only with no raw database fallback", async () => {
  ...
  for (const marker of ['crypto.subtle.digest("SHA-256"', '.eq("Token_Hash", tokenHash)', 'SESSION_EXPIRED']) {
  ...
  if (auth.includes('.eq("Token", token)') || auth.includes('Phase 8A compatibility')) {
    throw new Error("shared auth must not fall back to raw Sessions.Token after final cutover");
  }
});

Deno.test("SessionGateway transforms only real raw session tokens and rejects stored digests from public callers", async () => {
...
Deno.test("browser and public contract verification route legacy calls through SessionGateway", async () => {
  ...
  for (const marker of [
    ...
    '__HADIRLY_SESSION_GATEWAY_FETCH__',
    "'AbsenV2','AttendanceLocation','PayrollUser','ProfileOps','DeviceTrust'",
    "JSON.stringify({ target, payload })",
    "credentials: 'omit'",
    "referrerPolicy: 'no-referrer'",
  ]) {
    if (!config.includes(marker)) throw new Error(`frontend session gateway missing ${marker}`);
  }
  if (!verifyContract.includes('/functions/v1/SessionGateway') || !verifyContract.includes("target:'EmploymentContracts'")) {
    throw new Error("public contract verification must use SessionGateway");
  }
});
...
Deno.test("production deployment keeps public gateway aliases separate from JWT-verified internal cores", async () => {
  ...
  for (const marker of [
    ...
    '"AbsenV2Core"',
    '"EmploymentContractsCore"',
    '$GatewayAliases',
    'Set-Content -LiteralPath $AliasIndex -Value $GatewaySource',
    '--no-verify-jwt',
    'Men-deploy Edge Function internal',
  ]) {
    if (!deploy.includes(marker)) throw new Error(`deployment session cutover missing ${marker}`);
  }
});
