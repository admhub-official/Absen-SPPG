...
Deno.test("Data Users and operational dashboard are served safely by OperationsV2", async () => {
  ...
  for (const marker of [
    ...
    'action === "getOperationalDashboardV2"',
  ...
  if (endpoint.includes('select("ID_User,Jenis_Absen,Jam,Tanggal")') || endpoint.includes('.order("Jam"')) {
    throw new Error("Data Users must use the real Absensi.Waktu_Timestamp column, not legacy Jam");
  }
  if (endpoint.includes('db.from("Users").select("*")')) {
    throw new Error("Data Users must never expose every Users column");
  }
  for (const secret of ["Password_Hash", "Password_Salt", "Token_Reset_Password", "Face_Descriptor_JSON"]) {
    ...
    if (safeColumns.includes(secret)) throw new Error(`sensitive Users field leaked into operational response: ${secret}`);
  }
  ...
  for (const marker of [
    ...
    "'getOperationalUsersV2','getOperationalDashboardV2'",
