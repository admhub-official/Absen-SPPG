...
Deno.test("profile preview and downloaded PDF share one stable 300 DPI CR80 master raster", async () => {
  ...
  if(!bootstrap.includes("const VERSION = '26.11.45'"))throw new Error("bootstrap asset version must match current release");
  ...
  if(!serviceWorker.includes("const APP_VERSION = '26.11.45'")||!serviceWorker.includes("const CACHE = 'absen-sppg-hadirly-v86'"))throw new Error("service worker version/cache must match bootstrap");
  ...
  if(!config.includes("import('./src/app/bootstrap.js?v=26.11.45')"))throw new Error("top-level bootstrap import must match current release");
