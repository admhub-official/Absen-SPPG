...
Deno.test("profile preview and downloaded PDF share one stable 300 DPI CR80 master raster", async () => {
  ...
  if(!/const version = '\d+\.\d+\.\d+';/.test(release)||!/const cacheName = 'absen-sppg-hadirly-v\d+';/.test(release))throw new Error("shared release version/cache contract mismatch");
