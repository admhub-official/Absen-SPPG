...
Deno.test("profile preview and downloaded PDF share one stable 300 DPI CR80 master raster", async () => {
  const controller=await read("src/app/digital-id-card.js"),renderer=await read("src/app/digital-id-card-master-renderer.js"),css=await read("src/styles/pages/digital-id-card.css"),rendererCss=await read("src/styles/pages/digital-id-card-master-renderer.css"),bootstrap=await read("src/app/bootstrap.js"),serviceWorker=await read("sw.js"),config=await read("supabase-config.js");
  ...
  if(!bootstrap.includes("const VERSION = '26.11.46'"))throw new Error("bootstrap asset version must match current release");
  ...
  if(!serviceWorker.includes("const APP_VERSION = '26.11.46'")||!serviceWorker.includes("const CACHE = 'absen-sppg-hadirly-v87'"))throw new Error("service worker version/cache must match bootstrap");
  ...
  if(!config.includes("import('./src/app/bootstrap.js?v=26.11.46')"))throw new Error("top-level bootstrap import must match current release");
