...
Deno.test("front ID Card renders Kepala SPPG signature and keeps PDF canvas in sync", async () => {
  ...
  if (!bootstrap.includes("const VERSION = '26.11.41'")) throw new Error("frontend version must match current release");
  if (!sw.includes("absen-sppg-hadirly-v82") || !sw.includes("id-card-front-signature-renderer.js")) {
