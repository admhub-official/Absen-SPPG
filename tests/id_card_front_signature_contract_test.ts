...
Deno.test("front ID Card renders Kepala SPPG signature and keeps PDF canvas in sync", async () => {
  ...
  if (!release.includes("version = '26.11.55'") || !release.includes("cacheName = 'absen-sppg-hadirly-v96'")) throw new Error("frontend release/cache must match current release");
