...
Deno.test("front ID Card renders Kepala SPPG signature and keeps PDF canvas in sync", async () => {
  ...
  if (!bootstrap.includes("const VERSION = '26.11.40'")) throw new Error("frontend version was not bumped");
  if (!sw.includes("absen-sppg-hadirly-v81") || !sw.includes("id-card-front-signature-renderer.js")) {
