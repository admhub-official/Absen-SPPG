...
foreach ($FunctionName in $ObsoleteFunctions) { Write-Host "Menghapus Edge Function lama $FunctionName..."; & $SupabaseCli functions delete $FunctionName --project-ref $ProjectRef --yes; if ($LASTEXITCODE -ne 0) { Write-Warning "Fungsi $FunctionName tidak ditemukan atau sudah dihapus." } }

$FunctionNames = @(
  "AbsenCore","AttendanceLocation","Absen","AbsenV2","PayrollUser","ProfileOps","DeviceTrust","SecurityOps","ProductionReadiness","AttendanceCorrections","AttendanceImport","OperationsV2","Complaints","DigitalIdentity","EmploymentContracts","WorkforceOps","PlatformOps"
)
